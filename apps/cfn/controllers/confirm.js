...
const static_police_forces = require('../../../assets/static_police_forces');
...
module.exports = class ConfirmController extends BaseConfirmController {
  ...
  saveValues(req, res, callback) {
   ...
   _.first(data).fields.unshift({
    ...
    var destination_email = static_police_forces.allPoliceForcesEmailAddresses.filter(
                                function(itm) { return itm.value ==  req.sessionModel.get('police-force')}
                            )[0].email;
    
    Promise.all([
  //    this.sendEmail(config.email.caseworker, 'caseworker', data, req, res),
      this.sendEmail(destination_email, 'caseworker', data, req, res),
      this.sendEmail(req.sessionModel.get('originator-email'), 'customer', data, req, res)
